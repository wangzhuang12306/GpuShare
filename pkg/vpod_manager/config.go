...
func StartConfigManager(stopCh <-chan struct{}, kc kubernetes.Interface) error {
	...
	netListen, err := net.Listen("tcp", "0.0.0.0:9797") //监听IP:port
	if err != nil {
	...
	for {
		conn, err := netListen.Accept() //接收到ip:port的信息
		if err != nil {
...
func clientHandler(conn net.Conn) {
	...
	nodeName := ""
	pod, err := kubeClient.CoreV1().Pods("kube-system").Get(context.Background(), hostname, metav1.GetOptions{})
...
func UpdateNodeGPUInfo(nodeName string, uuid2mem *map[string]string) {
	...
	gpuinfo := buf.String()
	newNode.ObjectMeta.Annotations[gpusharev1.GPUShareNodeGPUInfo] = gpuinfo
	klog.Infof("Update node %s GPU info: %s", nodeName, gpuinfo)
	_, err = kubeClient.CoreV1().Nodes().Update(context.Background(),newNode,metav1.UpdateOptions{})
	if err != nil {
...
func syncConfig(nodeName, UUID string, podList *list.List) error {
	...
	if podList != nil {
		for pod := podList.Front(); pod != nil; pod = pod.Next() {
			...
			buf.WriteString(" ")
			buf.WriteString(fmt.Sprintf("%d", pod.Value.(*PodRequest).Priority))
			buf.WriteString(",")
		}
	}
